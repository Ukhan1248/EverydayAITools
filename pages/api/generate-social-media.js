...
export default async function handler(req, res) {
  const { subject, keywords, hashtags, email } = req.body;
  ...
  const response = await openai.createCompletion({
    ...
    prompt: `Generate a social media post with the following info.\n\nsubject: ${subject} keywords: ${keywords} hashtags: ${hashtags}`,
    temperature: 0,
    max_tokens: 400,
    top_p: 1,
