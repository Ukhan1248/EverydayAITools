...
export default async function handler(req, res) {
  const { title, date, location, topics, attendees, email } = req.body;
  ...
  const response = await openai.createCompletion({
    ...
    prompt: `Generate a Group Event Email with the following info.\n\nMeeting Title: ${title} Date: ${date} Location: ${location} Topics: ${topics} Attendees: ${attendees}`,
    temperature: 0,
    max_tokens: 400,
    top_p: 1,
