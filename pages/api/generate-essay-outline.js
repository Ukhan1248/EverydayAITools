...
export default async function handler(req, res) {
  const { essaykeywords, essaytitle, email } = req.body;
  ...
  const response = await openai.createCompletion({
    ...
    prompt: `Write an outline for an essay with the details below.\n\nEssay Title: ${essaytitle} Keywords: ${essaykeywords} also place in good format`,
    temperature: 0,
    max_tokens: 300,
    top_p: 1,
