...
export default async function handler(req, res) {
  const { title, heading, keywords, email } = req.body;
  ...
  const response = await openai.createCompletion({
    ...
    prompt: `Create the section for an Article with the below details. \n\nArticle Title: ${title}; Section Heading: ${heading}; Section Keywords: ${keywords}`,
    temperature: 0.8,
    max_tokens: 1000,
    top_p: 1,
